            ...
            >
              ...
              <div className="text-right">
                {parseFloat(client.solde_avoir) > 0 ? (
                  <div className="flex items-center gap-1 bg-orange-100 text-orange-700 px-2 py-1 rounded-lg mb-1">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-xs font-semibold">{parseFloat(client.solde_avoir).toFixed(2)} EUR</span>
                  </div>
                ) : (
                  <span className="text-xs text-gray-300">Pas d'avoir</span>
                )}
                <p className="text-xs text-gray-400">
                  {client.commandes?.[0]?.count || 0} cmd
                </p>
              </div>
            </button>
          ))}
